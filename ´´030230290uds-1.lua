...
local function enableSilentAim()
	if getfenv().hookmetamethod and not silentHook then
		silentHook = hookmetamethod(game, "__index", newcclosure(function(self, key)
			if not checkcaller() and aiming and self == mouse and (key == "Hit" or key == "Target") and target and target.Character then
...
aimlockButton.MouseButton1Click:Connect(function()
	aiming = not aiming
	aimlockButton.Text = aiming and "Aimlock AUS" or "Aimlock AN"
	if aiming then enableSilentAim() end
end)

RunService.Heartbeat:Connect(function()
	if aiming and tick() - lastUpdate >= aimUpdateInterval then
